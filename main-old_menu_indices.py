...
MENU_INDICES = [idle_index, burst_index, rev_index]
...
def menu():
    ...
    print(f"{MENU_ITEMS[master_index]} = {INDEXED_OPTIONS[master_index][MENU_INDICES[master_index]]}")
    while ENC.value:
        position = ENCODER.position
        ...
        if not DOWN.value:
            sleep(.1)
            master_index += 1
            if master_index == len(MENU_INDICES):
                master_index = 0
            print(f"{MENU_ITEMS[master_index]} = {INDEXED_OPTIONS[master_index][MENU_INDICES[master_index]]}")
        # If up button is pressed, move up the parameter list
            ...
            master_index -= 1
            if master_index < 0:
                master_index = len(MENU_INDICES) - 1
            print(f"{MENU_ITEMS[master_index]} = {INDEXED_OPTIONS[master_index][MENU_INDICES[master_index]]}")
        if position != last_position:
            # If encoder spins right/down, cycle right/down in the indexed parameter values
            if position > last_position:
                MENU_INDICES[master_index] += 1
                if MENU_INDICES[master_index] == len(INDEXED_OPTIONS[master_index]):
                    MENU_INDICES[master_index] = 0
            # If encoder spins left/up, cycle left/up in the indexed parameter values
            elif position < last_position:
                MENU_INDICES[master_index] -= 1
                if MENU_INDICES[master_index] < 0:
                    MENU_INDICES[master_index] = len(INDEXED_OPTIONS[master_index]) - 1
            print(f"{MENU_ITEMS[master_index]} = {INDEXED_OPTIONS[master_index][MENU_INDICES[master_index]]}")
            last_position = position
...
def idle_loop():
    ...
    escIdle = IDLE_SPEED[MENU_INDICES[0]]
